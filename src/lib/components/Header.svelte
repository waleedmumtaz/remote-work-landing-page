...
<header>
	...
	{#if isNavOpen}
		<nav
			transition:fly={{ x: 300, duration: 1000, opacity: 1 }}
			class:toggle-nav={isNavOpen ? '' : 'toggle-nav'}
		>
			<button on:click|preventDefault={toggleNav} class="close-button">
				...
				<li class="features">
					...
					{#if isFeaturesOpen}
						<ul transition:slide class:toggle-features={isFeaturesOpen ? '' : 'toggle-features'}>
							...
							<li>
								<span><img src="/icon-calendar.svg" alt="Calendar" /></span><a href="/#">Calendar</a
								>
							</li>
							<li>
							...
							<li>
								<span><img src="/icon-planning.svg" alt="Planning" /></span><a href="/#">Planning</a
								>
							</li>
						</ul>
					{/if}
